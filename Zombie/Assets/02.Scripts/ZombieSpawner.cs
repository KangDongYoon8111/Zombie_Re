...
public class ZombieSpawner : MonoBehaviour
{
    ...
    [SerializeField]private Transform[] spawnPoints; // 좀비 AI를 소환할 위치
    private List<Zombie> zombies = new List<Zombie>(); // 생성된 좀비를 담는 리스트
    private int wave; // 현재 웨이브

    private bool invokeStart = false;
    ...
    void Update()
    {
        ...
        if (zombies.Count <= 0 && !invokeStart)
        {
            invokeStart = true;
            Invoke("SpawnWave", 3f);
        }
    ...
    private void SpawnWave()
    {
        ...
        // 현재 웨이브 * 1.5를 반올림함 수만큼 좀비 생성
        int spawnCount = Mathf.RoundToInt(wave * 1.5f);

        // spawnCount만큼 좀비 생성
        for(int i = 0; i < spawnCount; i++)
        {
            CreateZombie(); // 좀비 생성 처리 실행
        }
    }
